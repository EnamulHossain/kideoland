            ...
            <div class="row">
                ...
                <div class="col-8 mt-4">
                    <div class="badges ">
                        <div class="d-flex">
                            ...
                            <div>
                                <span
                                    class="badge badge-inline preorder-border-dashed p-3 m-2 rounded-3 text-muted fs-12 fw-600">{{
                                    translate('Published') }}({{ $publishedProductCount }})</span>
                                <span
                                    class="badge badge-inline preorder-border-dashed p-3 m-2 rounded-3 text-muted fs-12 fw-600">{{
                                    translate('Unpublished') }}({{ $unpublishedProductCount }})</span>
                                <span
                                    class="badge badge-inline preorder-border-dashed p-3 m-2 rounded-3 text-muted fs-12 fw-600">{{
                                    translate('Discounted') }}({{ $discountedProductCount }})</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 mt-4">
                    <div class="row ml-0">
                        ...
                        <div class="col-lg-2">
                            <select class="form-control form-control-sm aiz-selectpicker" name="type"
                                ...
                                <option value="unit_price,desc" @isset($col_name , $query) @if($col_name=='unit_price'
                                    && $query=='desc' ) selected @endif @endisset>{{translate('Base Price (High >
                                    Low)')}}</option>
                                <option value="unit_price,asc" @isset($col_name , $query) @if($col_name=='unit_price' &&
                                    $query=='asc' ) selected @endif @endisset>{{translate('Base Price (Low > High)')}}
                                </option>
                            </select>
                        </div>
                        <div class="col-lg-4">
                        ...
                        <div class="col-auto">
                            <div class="form-group mb-0">
                                <button type="submit" class="btn btn-sm btn-soft-primary text-primary fw-700">{{
            ...
            <table class="table aiz-table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="mb-2">
                                <span class="text-muted text-truncate-3 break-word ">{{
                                    Str::limit($product?->getTranslation('product_name'), 50, ' ...') }}</span>
                            </div>
                            ...
                            <div>
                                <span class="text-muted text-truncate-2 text-info">{{ translate(
                                    $product->user->user_type != 'seller' ? 'In-house' : $product->user->shop->name)
                                    }}</span>
                                <span class="text-muted text-truncate-2 fw-700">{{ translate('Product Created :') .
                                    $product->created_at->format('d.m.Y') }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Min Purchase
                                    Qty') }}</span>
                                <span class="text-muted text-truncate-2 fw-700 fs-13">{{ $product->min_qty . ' ' .
                                    $product->unit }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Refund')
                                    }}</span>
                                <span class="text-muted text-truncate-2 fw-700 fs-13">{{ $product->is_refundable ?
                                    'Refundable' : 'Not Refundable' }}</span>
                            </div>
                        </td>

                        <td>
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Price')
                                    }}</span>
                                <span class="text-muted text-truncate-2 fw-700 fs-13">{{ $product->unit_price . ' / ' .
                                    $product->unit }}
                                    <span
                                        class="badge badge-inline badge-soft-success fs-13 fw-600 p-2 rounded-pill ml-1">{{
                                        translate('Fixed') }}</span>
                                </span>
                            </div>
                            @if($product->is_prepayment)
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Prepayment')
                                    }}</span>
                                <span class="text-muted text-truncate-2 fw-700 fs-13">{{
                                    $product->preorder_prepayment?->prepayment_amount }}
                                    <span class="badge badge-inline badge-soft-primary fs-13 fw-600 p-2 rounded-pill">{{
                                        translate('Needed') }}</span>
                                </span>
                            </div>
                            @endif
                        </td>
                        <td>
                            ...
                            <div class="bg-soft-primary px-4 py-2 rounded-1 mt-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12 text-primary">
                                    -{{ $product->discount_type == 'flat' ? single_price($product->discount) :
                                    $product->discount.'%' }}
                                </span>
                            </div>
                            @endif
                        ...
                        <td>
                            ...
                            <div class="bg-soft-secondary px-4 py-2 rounded-1 mt-2">
                                ...
                                <span class="text-muted text-truncate-2 fw-700 fs-13 text-success">{{
                                    translate('Available now') }}</span>
                                @elseif($product->available_date != null)

                                <span class="opacity-60 text-muted text-truncate-2 fs-12 text-secondary">{{
                                    $product->available_date }}</span>
                                <span
                                    class="text-muted text-truncate-2 fw-700 fs-13 text-secondary">{{strtotime($product->available_date)
                                    > strtotime(date('d-m-Y')) ? translate('Approx') : translate('Available now')
                                    }}</span>
                                @endif
                            </div>
                            @endif
                        ...
                        <td>
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('PreOrder')
                                    }}</span>
                                <span class="text-muted text-truncate-2 fw-700 fs-13">
                                    @if($product->is_prepayment)
                                    {{
                                    ...
                                    @else
                                    {{
                                    $product->preorder->where('request_preorder_status',2)->whereIn('final_order_status',[0,
                                    1])->count() }}
                                    @endif
                                </span>
                            </div>
                            <div>
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Final Order')
                                    }}</span>
                                <span class="text-muted text-truncate-2 fw-700 fs-13">{{
                                    $product->preorder->where('final_order_status', 2)->count() }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Publish')
                                    }}</span>
                            ...
                            <div class="mb-2">
                                <span class="opacity-60 text-muted text-truncate-2 fs-12">{{ translate('Featured')
                                    }}</span>
                                <label class="aiz-switch aiz-switch-success mb-0 mt-2">
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0 mt-2">
                                <input onchange="update_approval(this)" value="{{ $product->id }}" type="checkbox" @if
                                    ($product->is_approved == 1) checked @endif>
                                <span class="slider round"></span>
                            </label>
                        </td>
                        @endif
                        <td class="text-right">
                            <a class="btn btn-soft-success btn-icon btn-circle btn-sm"
                                href="{{ route('preorder-product.details', $product->product_slug) }}" target="_blank"
